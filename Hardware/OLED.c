...
void OLED_ShowChar(int16_t X, int16_t Y, char Char, uint8_t FontSize)
{
	...
	else if(FontSize == OLED_6X12)	//字体为宽6像素，高8像素
	{
...
void OLED_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
	...
	while (String[i] != '\0')	//遍历字符串
	{
		...
		else					//否则，即多字节字符
		{
            ...
            if (FontSize == OLED_8X16)//如果是8*16大小的字符，使用16*16的中文字符库
            {
            ...
            else if (FontSize == OLED_6X12)//给定字体为6*12点阵，使用12*12的中文字符库
            {
			...
			{
				/*空间不足，此位置显示'?'*/
				OLED_ShowChar(X + XOffset, Y, '?', OLED_6X12);
				XOffset += OLED_6X12;
			}
		}
	}
...
void OLED_ShowStringpl(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
	uint16_t i = 0;
	char SingleChar[5];
	uint8_t CharLength = 0;
	...
	while (String[i] != '\0')	//遍历字符串
	{
		...
		{
			/*使用OLED_ShowChar显示此字符*/
			OLED_ShowChar(X + XOffset, Y + YOffset, SingleChar[0], FontSize);
			XOffset += FontSize;
			if(X + XOffset >= OLED_XMAX - FontSize)
			{
				XOffset = 0;
				if (FontSize == OLED_8X16)
					YOffset += 16;
				if (FontSize == OLED_6X12)
					YOffset += 8;
				if (FontSize == OLED_12X24)
					YOffset += 24;
			}
		}
		else					//否则，即多字节字符
		{
            ...
            if (FontSize == OLED_8X16)//如果是8*16大小的字符，使用16*16的中文字符库
            {
				...
				/*将字模库OLED_CF16x16的指定数据以16*16的图像格式显示*/
				OLED_ShowImage(X + XOffset,Y + YOffset, 16, 16, OLED_CF16x16[pIndex].Data);
				XOffset += 16;
				if(X + XOffset >= OLED_XMAX - 16)
				{
					XOffset = 0;
					YOffset += 16;
				}
            ...
            else if (FontSize == OLED_6X12)//给定字体为6*8点阵，使用12*12的中文字符库
            {
                for (pIndex = 0; strcmp(OLED_CF12x12[pIndex].Index, "") != 0; pIndex ++)
                {
                    /*找到匹配的字符*/
                ...
                OLED_ShowImage(X + XOffset,Y + YOffset, 12, 12, OLED_CF12x12[pIndex].Data);
				XOffset += 12;
				if(X + XOffset >= OLED_XMAX - 12)
				{
					XOffset = 0;
					YOffset += 12;
				}
...
void OLED_Printfpl(int16_t X, int16_t Y, uint8_t FontSize, char *format, ...)
{
	char String[256];						//定义字符数组
	va_list arg;							//定义可变参数列表数据类型的变量arg
	va_start(arg, format);					//从format开始，接收参数列表到arg变量
	vsprintf(String, format, arg);			//使用vsprintf打印格式化字符串和参数列表到字符数组中
	va_end(arg);							//结束变量arg
	OLED_ShowStringpl(X, Y, String, FontSize);//OLED显示字符数组（字符串）
}
